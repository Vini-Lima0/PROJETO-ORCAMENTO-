...
interface Props {
  onLogin: (email: string, role: 'admin' | 'operacional') => void;
}
...
export default function Login({ onLogin }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setTimeout(() => {
      const role = email.includes('operacional') ? 'operacional' : 'admin';
      onLogin(email, role);
    }, 700);
  };
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <div style={{
            width: 52, height: 52, background: 'var(--text)', borderRadius: 14,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            margin: '0 auto 16px', fontFamily: "'Syne',sans-serif", fontSize: 18, fontWeight: 800, color: '#fff',
          }}>OP</div>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: 14 }}>
              ...
              <input
                ...
                style={{ width: '100%', padding: '10px 14px', border: '1px solid var(--border)', borderRadius: 10, fontSize: 14, fontFamily: "'DM Sans',sans-serif", outline: 'none', color: 'var(--text)', background: 'var(--surface)', transition: 'border .15s' }}
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ textAlign: 'right', marginTop: 6 }}>
                <button type="button" style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 12.5, color: 'var(--blue)', fontFamily: "'DM Sans',sans-serif" }}>Esqueci minha senha</button>
              </div>
            </div>

            {erro && (
              <div style={{ padding: '10px 14px', background: 'var(--red-bg)', color: 'var(--red)', borderRadius: 9, fontSize: 13, marginBottom: 14 }}>
                {erro}
              </div>
            )}

            <button type="submit" disabled={loading} style={{
              width: '100%', padding: '12px', background: loading ? 'var(--surface3)' : 'var(--text)',
              color: loading ? 'var(--text2)' : '#fff', border: 'none', borderRadius: 10,
              fontSize: 14, fontWeight: 600, fontFamily: "'DM Sans',sans-serif", cursor: loading ? 'not-allowed' : 'pointer',
              transition: 'all .15s',
            }}>
