...
export function gerarPDF(orc: Orcamento, config: ConfigEmpresa = defaultConfig) {
  ...
  // ── 1. TOP BAR: data (esq) | Orçamento NÚMERO (dir) ────────────────────────
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  doc.setTextColor(...C.gray);
  doc.text(fmtData(orc.criadoEm), M, y);

  // "Orçamento" normal + número bold grande — baseline alinhado
  ...
  // ── 2. CABEÇALHO: logo + empresa (esq) | telefone / email (dir) ─────────────
  const headerTop = y;
  let companyX = M;
  const logoSize = 22;
  ...
  // Endereço (pode ser longo — quebra em até 2 linhas)
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(...C.mid);
  let infoY = y + 11;
  ...
  // Telefone e e-mail — canto direito
  if (config.telefone) {
    doc.setFont('helvetica', 'bold');
  ...
  // Contato / info adicional do cliente
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(...C.mid);
  ...
  // Total (label cinza, valor bold escuro)
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  doc.setTextColor(...C.gray);
