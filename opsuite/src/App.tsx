...
export default function App() {
  ...
  return (
    <div style={{display:'flex',minHeight:'100vh'}}>
      ...
      <nav style={{
        width:240,background:'var(--text)',minHeight:'100vh',display:'flex',flexDirection:'column',
      ...
      <main style={{marginLeft:isMobile?0:240,flex:1,padding:isMobile?'20px 16px':'26px 32px',maxWidth:'100%',overflowX:'hidden',minWidth:0}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:26,flexWrap:'wrap',gap:12}}>
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            {isMobile && (
              <button onClick={()=>setSidebarOpen(true)} style={{width:36,height:36,borderRadius:9,border:'1px solid var(--border)',background:'var(--surface)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,color:'var(--text)'}}>☰</button>
            )}
            <div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            ...
            <div style={{width:36,height:36,borderRadius:9,border:'1px solid var(--border)',background:'var(--surface)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',position:'relative',fontSize:16}}>
              🔔
              {pendAlta > 0 && <span style={{position:'absolute',top:7,right:8,width:6,height:6,borderRadius:'50%',background:'var(--green)',border:'1.5px solid var(--surface)'}} />}
            </div>
            <button onClick={()=>{setEditOrc(null);navTo('novo-orcamento');}} style={{display:'flex',alignItems:'center',gap:7,padding:'9px 16px',borderRadius:10,background:'var(--text)',color:'#fff',border:'none',cursor:'pointer',fontSize:13.5,fontWeight:500,fontFamily:"'DM Sans',sans-serif",whiteSpace:'nowrap'}}>
              + Novo orçamento
            </button>
          </div>
        </div>
